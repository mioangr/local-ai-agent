...
| `repo_manager.py` | Small utility script for listing, adding, and removing repository definitions in `settings/repos/repos.json`. |
| `README.md` | Folder-local documentation for the agent worker. |

## Runtime Notes

- The agent consumes tasks from Redis queue `task:queue`
- The agent looks up repositories from `settings/repos/repos.json`
